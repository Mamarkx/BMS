            ...
            <div class="overflow-x-auto">
               <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-200">
                    <tr>
                        <th class="px-6 py-4 text-left text-sm font-medium text-gray-600 uppercase tracking-wider">ID</th>
                        <th class="px-6 py-4 text-left text-sm font-medium text-gray-600 uppercase tracking-wider">Full Name</th>
                        <th class="px-6 py-4 text-left text-sm font-medium text-gray-600 uppercase tracking-wider">Gender</th>
                        <th class="px-6 py-4 text-left text-sm font-medium text-gray-600 uppercase tracking-wider">Civil Status</th>
                        <th class="px-6 py-4 text-left text-sm font-medium text-gray-600 uppercase tracking-wider">Birthday</th>
                        <th class="px-6 py-4 text-left text-sm font-medium text-gray-600 uppercase tracking-wider">Status</th>
                        <th class="px-6 py-4 text-center text-sm font-medium text-gray-600 uppercase tracking-wider">Actions</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200" id="residentTableBody">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold {{ $resident->status == 'Active' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600' }}">
                                {{ $resident->status }}
                            </span>
                        </td>
                        <td class="px-2 py-4 whitespace-nowrap text-center">
                            <div class="flex justify-center gap-4">
                                <!-- View Button -->
                                <button onclick='viewRecord(@json($resident))' class="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:outline-none transition-all duration-200" title="View Details">
                                    <i class="fa-solid fa-eye text-base"></i>
                                    <span class="text-sm font-medium">View</span>
                                </button>

                                <!-- Edit Button -->
                                <button class="EditResidentInfo flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 focus:outline-none transition-all duration-200" 
                                ...
                                <!-- Delete Button -->
                                <form id="deleteForm_{{ $resident->resident_id }}" action="{{ route('DeleteResident', $resident->resident_id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" 
    ...
    <dialog id="my_modal_1" class="modal">
        <div class="modal-box bg-white rounded-2xl shadow-2xl w-full max-w-5xl p-0 max-h-[90vh] overflow-hidden">
            ...
            <div class="overflow-y-auto max-h-[calc(90vh-140px)] px-8 py-6">
                <form action="{{ route('addResident') }}" method="POST" id="addResidentForm" class="space-y-8" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                            <div class="space-y-2">
                                <label for="fname" class="block text-sm font-semibold text-slate-700">First Name <span class="text-red-500">*</span></label>
                            ...
                            <div class="space-y-2">
                                <label for="mname" class="block text-sm font-semibold text-slate-700">Middle Name</label>
                            ...
                            <div class="space-y-2">
                                <label for="lname" class="block text-sm font-semibold text-slate-700">Last Name <span class="text-red-500">*</span></label>
                            ...
                            <div class="space-y-2">
                                <label for="sex" class="block text-sm font-semibold text-slate-700">Gender <span class="text-red-500">*</span></label>
                                <select id="sex" name="sex" 
                                        class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200" required>
                            ...
                            <div class="space-y-2">
                                <label for="birth_date" class="block text-sm font-semibold text-slate-700">Birthdate <span class="text-red-500">*</span></label>
                                <input type="date" id="birth_date" name="birth_date" 
                                       class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200" required>
                            </div>

                            <div class="space-y-2">
                                <label for="civil_status" class="block text-sm font-semibold text-slate-700">Civil Status <span class="text-red-500">*</span></label>
                    ...
                    <div class="space-y-5">
                        <div class="flex items-center gap-3 pb-3 border-b-2 border-emerald-100">
                            ...
                            <div class="space-y-2">
                                <label for="lname" class="block text-sm font-semibold text-slate-700">Contact Number <span class="text-red-500">*</span></label>
                                <input type="text" id="contact_number" name="contact_number" 
                                       class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200" 
                                       placeholder="09122333" required>
                            </div>
                        <div class="space-y-2">
                            <label for="address" class="block text-sm font-semibold text-slate-700">Complete Address <span class="text-red-500">*</span></label>
...
<dialog id="viewModal" class="modal backdrop:bg-black/80 backdrop:backdrop-blur-sm">
  <div class="modal-box max-w-4xl bg-white rounded-xl shadow-2xl p-0 overflow-hidden transform transition-all duration-300">
    ...
    <div class="px-6 py-6 bg-gray-50 max-h-[70vh] overflow-y-auto space-y-6">
      ...
      <div class="bg-white rounded-lg shadow-sm p-6 flex flex-col sm:flex-row gap-6 items-center border border-gray-100">
        
        <div class="w-24 h-24 flex-shrink-0 bg-indigo-50 rounded-full flex items-center justify-center border-4 border-white shadow-md overflow-hidden">
          <i class="fa-solid fa-user text-indigo-500 text-4xl"></i>
        </div>

        <div class="flex-grow text-center sm:text-left">
          <h2 id="v_full_name_header" class="text-3xl font-extrabold text-gray-800 tracking-tight mb-1"></h2>
          <p class="text-gray-500 font-mono text-sm mb-2">Resident ID: <span id="v_resident_id">N/A</span></p>
          
          <p id="v_status_badge" class="inline-flex items-center px-4 py-1 text-sm font-semibold rounded-full transition"></p>
        </div>
      </div>
    ...
    <div class="bg-white px-6 py-4 flex justify-end border-t border-gray-100">
      <button onclick="$('#viewModal')[0].close()" class="px-6 py-2.5 bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold rounded-lg transition">
...
<script>
    ...
    function viewRecord(resident) {
        ...
        $('#v_full_name_header').text(resident.first_name + ' ' + (resident.middle_name ? resident.middle_name : '') + ' ' + resident.last_name);
    ...
    $('.delete-btn').on('click', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $('#deleteForm_' + id).submit();  // Submitting the correct form based on the resident's id
            }
        });
    });
});
