<x-admin-layout>
    <div class="p-6 bg-white rounded-2xl h-auto">
        <!-- Header -->
     ...
     <!-- DaisyUI Modal for Create -->
        <dialog id="createModal" class="modal">
            <div class="modal-box max-w-2xl">
                ...
                <form id="createAnnouncementForm" method="POST" action="{{ route('announce.store') }}" enctype="multipart/form-data" class="space-y-4">
                    @csrf

                    <div>
                        <label class="block font-medium text-sm mb-1">Title</label>
                        <input type="text" name="title" class="input input-bordered w-full" placeholder="Enter announcement title" required>
                    </div>

                    <div>
                        <label class="block font-medium text-sm mb-1">Content</label>
                        <textarea name="content" class="textarea textarea-bordered w-full" rows="4" placeholder="Write your announcement..." required></textarea>
                    </div>
                    ...
                    <div class="bg-white shadow-md rounded-xl overflow-hidden hover:shadow-lg transition relative">
                        ...
                        <div class="p-5 space-y-3">
                            <div class="flex justify-between items-start">
                                ...
                                <div class="flex space-x-3">
                                    <!-- Edit Button -->
                                    <button class="text-blue-600 hover:text-blue-800" onclick="document.getElementById('editModal-{{ $announcement->id }}').showModal()">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>

                                    <!-- Delete -->
                            <form action="{{ route('DeleteAnnounce', $announcement->id) }}" 
                                id="deleteForm_{{ $announcement->id }}" 
                                method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="button"  class="text-red-600 hover:text-red-800 delete-btn" data-id="{{ $announcement->id }}">
                        ...
                        <dialog id="editModal-{{ $announcement->id }}" class="modal">
                            <div class="modal-box max-w-2xl">
                                ...
                                <form method="POST" action="{{ route('announce.update', $announcement->id) }}" enctype="multipart/form-data" class="space-y-4">
                                    @csrf
                                    @method('PUT')
                                    <div>
                                        <label class="block font-medium text-sm mb-1">Title</label>
                                        <input type="text" name="title" value="{{ $announcement->title }}" class="input input-bordered w-full" required>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select name="category" class="select select-bordered w-full">
                                                ...
                                                <option {{ $announcement->category === 'Emergency' ? 'selected' : '' }}>Emergency</option>
                                                <option {{ $announcement->category === 'Health' ? 'selected' : '' }}>Health</option>
                                                <option {{ $announcement->category === 'Public Notice' ? 'selected' : '' }}>Public Notice</option>
                                            </select>
                                        </div>
                                    ...
                                    <div class="modal-action">
                                        ...
                                        <button type="button" class="btn" onclick="document.getElementById('editModal-{{ $announcement->id }}').close()">Cancel</button>
                                    </div>
                                </form>
                            </div>
                        </dialog>
